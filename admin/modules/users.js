...
async function loadUsers() {
    try {
        ...
        const usersQuery = firebase.firestore()
        ...
        snapshot.forEach(doc => {
            ...
            if (!userData.property) userData.property = currentUser.property;
...
async function createUser(email, password, name, role, property) {
    try {
        const { auth, db, COLLECTIONS, currentUser } = CommonUtils;
        ...
        await db.collection(COLLECTIONS.ADMINS).doc(firebaseUserId).set(userData);
        
        // Log activity
        await logUserActivity(firebaseUserId, 'created', currentUser);
...
async function updateUser(userId, updateData) {
    try {
        ...
        await db.collection(COLLECTIONS.ADMINS).doc(userId).update(updateObj);
        
        // Log activity
        await logUserActivity(userId, 'updated', currentUser);
    ...
    CommonUtils.showConfirm(
        ...
        async () => {
            try {
                ...
                await db.collection(COLLECTIONS.ADMINS).doc(userId).delete();
                
                // Log activity
                await logUserActivity(userId, 'deleted', currentUser);
