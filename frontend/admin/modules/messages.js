...
async function loadMessages() {
    try {
        ...
        let url = `${CommonUtils.API_URL}/messages`;
...
function setupRealtimeListener() {
    ...
    // Store interval ID
    window.messagesPollingInterval = pollInterval;
}
...
async function markAsRead(messageId) {
    try {
        const response = await fetch(`${CommonUtils.API_URL}/messages/${messageId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                read: true,
                readBy: CommonUtils.currentUser.name || 'Admin'
            })
        });
        ...
        if (messageIndex !== -1) {
            ...
            messagesData[messageIndex].readAt = new Date().toISOString();
            messagesData[messageIndex].readBy = CommonUtils.currentUser.name || 'Admin';
        }
        ...
        async () => {
            try {
                ...
                await Promise.all(unreadMessages.map(message => {
                    return fetch(`${CommonUtils.API_URL}/messages/${message.id}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            read: true,
                ...
                loadMessages(); // Refresh data from server
...
async function sendReply(event, messageId) {
    ...
    try {
        ...
        const replyData = {
            ...
            replied: true,
            repliedBy: CommonUtils.currentUser.name || 'Admin',
        };
...
async function sendNewMessage(event) {
    ...
    try {
        ...
        const response = await fetch(`${CommonUtils.API_URL}/messages`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
